/**
 * main.js
 * Application controller. Owns view transitions, event handling,
 * and the loading sequence.
 */

import { fetchAll }     from './api/index.js';
import { renderBrief }  from './components/brief.js';
import { getHistory, saveToHistory, clearHistory, formatDate } from './utils/history.js';
...
async function runSearch(rawTopic) {
  ...
  // Stagger stage dots for perceived progress
  STAGE_KEYS.forEach((k, i) => {
    setTimeout(() => {
      if (!stageEls[k].classList.contains('done')) stageEls[k].classList.add('active');
    }, i * 200);
  });
...
function flashError(msg) {
  ...
  searchBox.style.boxShadow   = '0 0 0 3px rgba(192,57,43,0.15)';
  searchInput.placeholder     = msg;
  setTimeout(() => {
    searchBox.style.borderColor = '';
    searchBox.style.boxShadow   = '';
    searchInput.placeholder     = 'e.g. neural networks, climate change, Rust programming...';
  }, 3000);
}
